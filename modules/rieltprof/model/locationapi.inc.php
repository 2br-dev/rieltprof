...
use \Rieltprof\Model\Orm\Location as Region;

class LocationApi extends TreeList
{
    const DEFAULT_REGION_NAME = "Краснодарский край";
    ...
    {
        parent::__construct(new \Rieltprof\Model\Orm\Location(), [
    ...
    /**
     * Возвращает список только регионов для админ панели
     *
     * @return array
     */
    static function regionsList(array $first = null)
    {
        $_this = new self();
        $_this->setFilter('parent_id', 0);
        $_this->setOrder('title');
    ...
    /**
     * Возвращает списко Регионов в виде массива
     * @return array
     */
    public function getRegions()
    {
        return $this
                ->setFilter('public', 1)
                ->setFilter('parent_id', 0)
                ->setOrder('title')
                ->getListAsArray();
    }

    /**
     * Возвращает списко городов в виде массива по заданному id региона
     * @param $region_id
     * @return array
     */
    ...
    /**
     * Получаем выбранный регион из сессии
     * @return bool|Region
     */
    public function getCurrentRegion()
    {
        if(isset($_SESSION['selected_region']) && !empty($_SESSION['selected_region'])){
            return new \Rieltprof\Model\Orm\Location($_SESSION['selected_region']);
        }
        return false;
    }
    ...
    public function getCurrentCity()
    {
        if(isset($_SESSION['selected_city']) && !empty($_SESSION['selected_city'])){
            return new \Rieltprof\Model\Orm\Location($_SESSION['selected_city']);
        }
        return false;
    }

    public function getParentLocation($parent_id)
    {
        return $this->setFilter('parent_id', $parent_id)->getFirst();
    }
}
