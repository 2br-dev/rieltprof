...
use \Catalog\Model\Orm\MultiOfferLevel;
use \Catalog\Model\Orm\Property;
...
//                'Checker' => array('chkEmpty', t('Укажите название товара')),
            ...
            'public' => new Type\Integer(array(
                'maxLength' => '1',
                'index' => true,
                'description' => t('Показывать объект на сайте'),
...
//                'Checker' => array('chkEmpty', t('Укажите хотя бы одну категорию')),
    ...
    {
        $this['district'] = @unserialize($this['_district']);
        $this['county'] = @unserialize($this['_county']);
        $this['state'] = @unserialize($this['_state']);
        $this['material'] = @unserialize($this['_material']);
        $this['rooms_list'] = @unserialize($this['_rooms_list']);
    ...
    function beforeWrite($flag, \RS\Event\Event $event)
    {
        ...
        $dir = 0;
        // В зависимости от action добавляем объект в нужную папку. Номера папок передаются в запросе
        if($action == 'sale'){
            $dir = $url->request('sale_dir', TYPE_INTEGER, 0);
        }
        if($action == 'rent'){
            $dir = $url->request('rent_dir', TYPE_INTEGER, 0);
        }
        // Тип объекта
        $object = $url->request('object', TYPE_STRING, '');
        ...
        if($this->isModified('district')){
            $this['_district'] = serialize($this['district']);
        }
        $this->addProperty($config['prop_district'], $this['district']);

        if($this->isModified('rooms_list')){
            $this['_rooms_list'] = serialize($this['rooms_list']);
        }
        $this->addProperty($config['prop_rooms_list'], $this['rooms_list']);
        $district = $this->getProductPropValue($config['prop_district'], 'district');
        $rooms = $this->getProductPropValue($config['prop_rooms_list'], 'rooms_list');
        if($rooms == 'Студия'){
            if($action == 'sale'){
                $this['title'] = $district .', '. $this['cost_product'] . 'р., ' . $object . ', ' . $rooms;
            }
            if($action == 'rent'){
                $this['title'] = $district .', '. $this['cost_rent'] . 'p./мес., ' . $object . ', ' . $rooms;
            }
        }else {
            if($action == 'sale'){
                $this['title'] = $district .', '. $this['cost_product'] . 'р., ' . $rooms . 'ком. ' . $object;
            }
            if($action == 'rent'){
                $this['title'] = $district .', '. $this['cost_rent'] . 'p./мес., ' . $rooms . 'ком. ' . $object;
            }
        }
        if($this->isModified('county')){
            $this['_county'] = serialize($this['county']);
        }
        $this->addProperty($config['prop_county'], $this['county']);
        if(!empty($this['street'])){
            $this->addProperty($config['prop_street'], $this['street']);
        }
        if(!empty($this['house'])){
            $this->addProperty($config['prop_house'], $this['house']);
        }
        if(!empty($this['liter'])){
            $this->addProperty($config['prop_liter'], $this['liter']);
        }
//        $this->addProperty($config['prop_rooms'], $this['rooms']);

        $this->addProperty($config['prop_isolated'], $this['rooms_isolated']);
        $this->addProperty($config['prop_split_wc'], $this['split_wc']);
        ...
        if($this['year']){
            $this->addProperty($config['prop_year'], $this['year']);
        }
        if($this->isModified('state')){
            $this['_state'] = serialize($this['state']);
        }
        $this->addProperty($config['prop_state'], $this['state']);
        $this->addProperty($config['prop_square'], $this['square']);
        $this->addProperty($config['prop_square_kitchen'], $this['square_kitchen']);
        $this->addProperty($config['prop_square_living'], $this['square_living']);
        $this->addProperty($config['prop_flat'], $this['flat']);
        $this->addProperty($config['prop_flat_house'], $this['flat_house']);
        if($this['flat'] == 1){
            $this['is_first'] = 1;
            $this['is_last'] = 0;
        }
        if ($this['flat'] == $this['flat_house']){
            $this['is_first'] = 0;
            $this['is_last'] = 1;
        }
        $this->addProperty($config['prop_is_first'], $this['is_first']);
        $this->addProperty($config['prop_is_last'], $this['is_last']);
        $this->addProperty($config['prop_quickly'], $this['quickly']);
        $this->addProperty($config['prop_mark'], $this['mark']);

        if($action == 'sale'){
            $this->addProperty($config['prop_only_cash'], $this['only_cash']);
            $this->addProperty($config['prop_mortgage'], $this['mortgage']);
            $this->addProperty($config['prop_breakdown'], $this['breakdown']);
            $this->addProperty($config['prop_encumbrance'], $this['encumbrance']);
            $this->addProperty($config['prop_child'], $this['child']);
            $this->addProperty($config['prop_remodeling'], $this['remodeling']);
            $this->addProperty($config['prop_remodeling_legalized'], $this['remodeling_legalized']);
        }

        $this->addProperty($config['prop_exclusive'], $this['exclusive']);
        $this->addProperty($config['prop_advertise'], $this['advertise']);
        if($this['cost_product']){
            $this->addProperty($config['prop_cost_product'], $this['cost_product']);
            $this['cost_one'] = $this['cost_product'] / $this['square'];
            $this->addProperty($config['prop_cost_one'], $this['cost_one']);
        }
        if($this['cost_rent']){
