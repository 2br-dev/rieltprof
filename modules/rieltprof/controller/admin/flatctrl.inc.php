...
class FlatCtrl extends Crud
{
    ...
    {
        parent::__construct(new FlatApi());
    ...
    public function actionAdd($primaryKeyValue = null, $returnOnSuccess = false, $helper = null)
    {
        ...
        if ($primaryKeyValue <= 0) {
            ...
            $this->getHelper()->setTopTitle(t('Добавить объект'));
        } else {
            $obj->fillCategories();
            $obj->fillCost();
            $obj->fillOffers();
            $obj->fillOffersStock();
            $this->getHelper()->setTopTitle(t('Редактировать объект ') . '{title}');
        }
