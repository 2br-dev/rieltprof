...
class Auth extends Front
{
    ...
    public function actionIndex()
    {
        ...
        if ($this->isMyPost()) {
            ...
            if ($user) {
                /**
                 * @var \Users\Config\File $config_user
                 */
                $config_user = \RS\Config\Loader::byModule('users');
                if ($config_user->isEnabledTwoFactorAuthorization($user)) {
            ...
            if($error == '' && !$user['access']){
                $error = t('Учетная запись не подтверждена Администратором');
            }
        }
        ...
        return $this->result->setTemplate('%users%/authorization.tpl');
    }
    ...
    public function actionVerify()
    {
        ...
        return $this->result
                        ->setSuccess(true)
                        ->setTemplate('%users%/authorization_two_step.tpl');
    }
    ...
    protected function actionByPhone()
    {
        ...
        return $this->result->setTemplate('%users%/authorization_by_phone.tpl');
    }
    ...
    function actionRecover()
    {
        ...
        return $this->result->setTemplate( '%users%/recover_pass.tpl' );
    }
    ...
    function actionChangePassword()
    {
        ...
        return $this->result->setTemplate( '%users%/change_pass.tpl' );
    }
