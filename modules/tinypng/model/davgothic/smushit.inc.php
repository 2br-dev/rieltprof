...
namespace TinyPNG\Model\DavGothic;
...
class SmushIt
{
    ...
    private function smushUrl()
    {
        ...
        curl_setopt($this->curl, CURLOPT_URL, self::SMUSH_URL . http_build_query(array('img' => $this->imageLocation, 'qlty' => $config['quality'])));
    ...
    private function smushFile()
    {
        if ( ! is_file($this->imageLocation) || ! is_readable($this->imageLocation)) {
            //throw new \rs\Exception('Could not read image file '.$this->imageLocation, 500);
            return false;
        }
