...
{else}
    <div class="user-without-register">
        <div class="form-group">
            ...
            {$order->getPropertyView('user_fio', ['placeholder' => "{t}Фамилия, Имя и Отчество покупателя, владельца аккаунта{/t}", 'class' => 'rs-checkout_triggerUpdate'])}
        </div>
        <div class="form-group">
            <label class="label-sup">{t}E-mail{/t}</label>
            {$order->getPropertyView('user_email', ['placeholder' => "{t}E-mail покупателя, владельца аккаунта{/t}", 'class' => 'rs-checkout_triggerUpdate'])}
        </div>
        <div class="form-group">
            <label class="label-sup">{t}Телефон{/t}</label>
            {$order->getPropertyView('user_phone', ['placeholder' => "{t}В формате: +7(123)9876543{/t}", 'class' => 'rs-checkout_triggerUpdate'])}
        </div>
    </div>
